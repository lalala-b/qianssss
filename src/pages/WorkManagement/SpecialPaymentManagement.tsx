...
import { Spin, Table, Button, message, ConfigProvider, Card } from "antd";
...
const SpecialPaymentManagement = () => {
  ...
  const handleSearch = () => {
    ...
    getStatisticsAmount({...newSearchData})
  };
  ...
  // 获取统计项
  function getStatisticsAmount(data?: any) {
    const params = {
      ...searchData,
      ...data,
      isExport: 0,
    };
    $getStatisticsAmount(formatParams(params))
      .then((res:any) => {
        const {
          total = 0,
          saleIncomeSum = 0,
          performanceMoneySum = 0,
          businessRevenueSum = 0,
        } = res || {};
        setPriceBoxList([
          {
            title: "数量",
  ...
  useEffect(() => {
    ...
    if (urlQuery.busOrderNo) {
      ...
      getStatisticsAmount({ busOrderNo: urlQuery.busOrderNo });
      return;
    }
