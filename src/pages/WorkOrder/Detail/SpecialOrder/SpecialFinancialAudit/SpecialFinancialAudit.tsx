...
import styles from "./SpecialFinancialAudit.scss";

interface SpecialFinancialAuditFormDataType {
  recePayType: number;
}

const SpecialFinancialAudit: React.FC = () => {
  const {
    detail: {
      orderBaseInfoBO: {
        // busOrderType = -1,
  ...
  const handleFinishForm = async (val: SpecialFinancialAuditFormDataType) => {
    try {
      setLoading(true);
      await $checkAccounts({
        ...val,
...
export default SpecialFinancialAudit;
