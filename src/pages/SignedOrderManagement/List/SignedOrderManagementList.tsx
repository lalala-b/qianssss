...
import TerminateOrderManagementList from "./TerminateOrderManagementList";
import CooperateOrderManagementList from "./CooperateOrderManagementList";
...
import styles from "./SignedOrderManagementList.scss";
...
const SignedOrderManagementList = () => {
  ...
  const [signOrderList, setSignOrderList] = useState<
  ...
  const [signedOrderActiveKey, setSignedOrderActiveKey] = useState<string>("1");
  ...
  function getTableList(extraParams: OrderListRequest = {}) {
    ...
    $getList({
      ...
      .then(res => {
        ...
        setSignOrderList([...(signOrderList || [])]);
        setPriceBoxList([
          // {
          //   title: '流水总额',
          //   value: allOrderMoney,
  ...
  // 切换签约订单tab页
  const handleChangeSignedOrderTab = (val: string) => {
    setSignedOrderActiveKey(val);
  };
  ...
  const getByDictType = async () => {
    const {
      perform_payment_type: performPaymentType,
  ...
  return (
    <Card>
      <Tabs
        ...
        activeKey={signedOrderActiveKey}
        onChange={handleChangeSignedOrderTab}
      >
        <TabPane tab="签约订单" key="1">
          <div>
            {searchConfig.length && (
              <Search
                ...
                onExport={
                  $permission("signed_order_manage_export")
                    ? onExport
                    : undefined
                }
                onSearch={onSearch}
            ...
            <div className={styles.checkboxWrap}>
              ...
              <Checkbox
                ...
                onChange={e =>
                  handleShowPerformedOrder(e, "weedCancelOrderFlag")
                }
              >
                剔除取消合作的订单
              </Checkbox>
        ...
        <TabPane tab="解约账号订单" key="2">
          <TerminateOrderManagementList />
        </TabPane>

        <TabPane tab="合作订单" key="3">
          <CooperateOrderManagementList />
        </TabPane>
      </Tabs>
    </Card>
  );
